...
import { View, Dimensions, Text, StyleSheet, TouchableOpacity, ActivityIndicator } from 'react-native';
...
export default class MyCoach extends React.Component {
 ...
 this.setState({coach_id:user.coach.coach_id})
    ...
    get_coach_by_id(user.coach.coach_id).then((res) => {
      this.setState({ coach: res.data })}).then(()=>{
  ...
  render() {
    const dayPreference = []
    return (
      this.state.loading==false?  <ActivityIndicator  size="large" color="#2CDEE4"/>:
      <View style={{ backgroundColor:'black', flex: 1 }}>
        ...
        <View
              ...
              <View style={{ width:widthPercentageToDP(100), marginTop:15,alignItems:'center'}}>
                <Avatar
                  size={100}
                  rounded
                  source={{
                ...
                />
                <Text style={{fontFamily:'MontserratSemiBold',fontSize:20,marginTop:20,color:'white'}}>{this.state.coach.first_name} {this.state.coach.last_name} </Text>
                <Text style={{fontFamily:'MontserratSemiBold',fontSize:13,marginTop:10,color:'#2CDEE4'}}>{this.state.coach.gym.name} {this.state.coach.gym.city} </Text>
                 </View>
            </View>
        ...
        <Text style={{justifyContent:'center',fontFamily:'RobotoBold',fontSize:18, marginRight:50}}>{this.state.coach.phone}</Text>
              <Image 
                 style={styles.Logo}
                 source={{
            ...
            <View
             ...
             <View
              ...
              <Text style={styles.text}> Ses diplome:</Text>
              <FlatList
                horizontal={true}
            ...
            <View
              ...
              <Text style={styles.textBlue}>{this.state.coach.experience_years} ans</Text>
            </View>
            <View
              style={{
                margin: 5,
