...
const ActivitiesCoachScreenView = ({ state, controller }) => {
  const [screen, setScreen] = useState(state.screen);

  const rightSwipe = (itemId, index) => (
    <TouchableOpacity
      style={styles.rightSwip}
      onPress={() => controller.onDeleteReminder(itemId, index)}>
    ...
    <View style={styles.container}>
      ...
      {screen === 'NOTIFICATIONS' ? null : (
        <TouchableOpacity
          ...
          onPress={controller.onCreateReminderPress}>
          <Image
      ...
      <View style={styles.content}>
        <View style={{ marginBottom: 28, alignItems: 'center' }}>
          <SwitchSelector
            ...
            onPress={(value) => {
              setScreen(value);
              controller.setState({ screen: value });
            }}
...
export default ActivitiesCoachScreenView;
