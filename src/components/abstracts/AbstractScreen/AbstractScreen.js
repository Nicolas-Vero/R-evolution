...
export default class AbstractScreen extends AbstractComponent {
  ...
  listenNavigationEvents = () => {
    this.willFocusNavigationEventListener = this.props.navigation.addListener(
      'willFocus',
      (payload) => {
        this.controller.abstractScreenWillFocus(payload);
      },
    );
    this.didFocusNavigationEventListener = this.props.navigation.addListener(
      'didFocus',
      (payload) => {
        this.controller.abstractScreenDidFocus(payload);
      },
    );
    this.willBlurNavigationEventListener = this.props.navigation.addListener(
      'willBlur',
      (payload) => {
        this.controller.abstractScreenWillBlur(payload);
      },
    );
    this.didBlurNavigationEventListener = this.props.navigation.addListener(
      'didBlur',
      (payload) => {
  ...
  removeNavigationEvents = () => {
    this.willFocusNavigationEventListener &&
      this.willFocusNavigationEventListener.remove();
    this.didFocusNavigationEventListener &&
      this.didFocusNavigationEventListener.remove();
    this.willBlurNavigationEventListener &&
      this.willBlurNavigationEventListener.remove();
    this.didBlurNavigationEventListener &&
